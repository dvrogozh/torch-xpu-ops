...
parser.add_argument('--json', action='store_true', help="Output tables in json format")
...
def print_md_row(row, print_header):
    if print_header:
        header = " | ".join([f"{key}" for key, _ in rn.items()])
        print(f"| {header} |")
        header = " | ".join(["-"*len(key) for key, _ in rn.items()])
        print(f"| {header} |")
    row = " | ".join([f"{value}" for _, value in rn.items()])
    print(f"| {row} |")
