...
struct RsqrtFunctor {
  scalar_t operator()(scalar_t a) const {
    SYCL_KERNEL_ASSERT(!"sycl::rsqrt is not supported");
...
struct Expm1Functor {
  scalar_t operator()(scalar_t a) const {
    using opmath_t = at::opmath_type<scalar_t>;
    SYCL_KERNEL_ASSERT(!"sycl::expm1 is not supported");
...
struct Expm1Functor<c10::complex<T>> {
  c10::complex<T> operator()(c10::complex<T> x) const {
    ...
    SYCL_KERNEL_ASSERT(!"sycl::expm1, cos, sin are not supported");
...
struct FrexpFunctor {
  std::tuple<scalar_t, int32_t> operator()(scalar_t a) const {
    using opmath_t = at::opmath_type<scalar_t>;
    int32_t exponent;
    SYCL_KERNEL_ASSERT(!"sycl::frexp is not supported");
