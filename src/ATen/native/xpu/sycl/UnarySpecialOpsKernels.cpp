...
struct SigmoidFunctor {
  scalar_t operator()(scalar_t a) const {
    ...
    if constexpr (c10::is_complex<opmath_t>::value) {
      ...
      SYCL_KERNEL_ASSERT(!"sycl::exp is not supported");
...
struct ErfFunctor {
  scalar_t operator()(scalar_t a) const {
    SYCL_KERNEL_ASSERT(!"sycl::erf is not supported");
...
struct ErfcFunctor {
  scalar_t operator()(scalar_t a) const {
    SYCL_KERNEL_ASSERT(!"sycil::erfc is not supported");
...
struct ErfcFunctor<double> {
  double operator()(double a) const {
    SYCL_KERNEL_ASSERT(!"sycl::erfc is not supported");
    //return sycl::erfc(a);
  }
};
...
struct Exp2Functor {
  scalar_t operator()(scalar_t a) const {
    using opmath_t = at::opmath_type<scalar_t>;
    SYCL_KERNEL_ASSERT(!"sycl::exp2 is not supported");
  ...
  scalar_t operator()(scalar_t a) const {
    if (a == scalar_t(0)) {
      ...
      if constexpr (c10::is_complex<opmath_t>::value) {
        ...
        SYCL_KERNEL_ASSERT(!"sycl::sin is not supported");
