...
inline c10::complex<T> reciprocal_wrapper(c10::complex<T> v) {
  ...
  auto both_inf = [](T real, T imag) {
    SYCL_KERNEL_ASSERT(!"sycl::isinf is not supported");
  ...
  auto either_nan = [](T real, T imag) {
    SYCL_KERNEL_ASSERT(!"sycl::isnan is not supported");
...
struct CeilFunctor {
  scalar_t operator()(const scalar_t a) const {
    using opmath_t = at::opmath_type<scalar_t>;
    SYCL_KERNEL_ASSERT(!"sycl::ceil is not supported");
...
inline scalar_t nearbyint_wrapper(scalar_t a) {
  SYCL_KERNEL_ASSERT(!"sycl::rint is not supported");
...
struct FloorFunctor {
  scalar_t operator()(scalar_t a) const {
    using opmath_t = at::opmath_type<scalar_t>;
    SYCL_KERNEL_ASSERT(!"sycl::floor is not supported");
