...
static inline C10_HOST_DEVICE scalar_t calc_digamma(scalar_t in) {
  ...
  if (x == accscalar_t(0)) {
    ...
    SYCL_KERNEL_ASSERT(!"sycl::copysign is not supported");
    //return static_cast<scalar_t>(
    //    sycl::copysign(static_cast<accscalar_t>(INFINITY), -x));
  }

  SYCL_KERNEL_ASSERT(!"sycl::trunc is not supported");
...
scalar_t ratevl(
  ...
  SYCL_KERNEL_ASSERT(!"sycl::fabs is not supported");
  ...
  if (absx > 1) {
    i = N - M;
    SYCL_KERNEL_ASSERT(!"sycl::pow is not supported");
...
static scalar_t _igam_helper_fac(scalar_t a, scalar_t x) {
  ...
  SYCL_KERNEL_ASSERT(!"sycl::fabs, log, lgamma, exp, sqrt, ... are not supported");
...
static scalar_t _igamc_helper_series(scalar_t a, scalar_t x) {
  ...
  SYCL_KERNEL_ASSERT(!"sycl::fabs, log, expm1, lgamma, exp are not supported");
  /*for (n = 1; n < MAXITER; n++) {
    fac *= -x / n;
    term = fac / (a + n);
    sum += term;
...
static const scalar_t _igam_helper_asymptotic_series(
  ...
  SYCL_KERNEL_ASSERT(!"sycl::sqrt, ... are not supported");
#if 0
  if (lambda > 1) {
...
static scalar_t _igamc_helper_continued_fraction(scalar_t a, scalar_t x) {
  ...
  SYCL_KERNEL_ASSERT(!"sycl::fabs, ... are not supported");
#if 0
  for (i = 0; i < MAXITER; i++) {
    c += 1.0;
    y += 1.0;
...
inline scalar_t calc_igammac(scalar_t a, scalar_t x) {
  ...
  SYCL_KERNEL_ASSERT(!"sycl::isinf, ... are not supported");
#if 0
...
static inline C10_HOST_DEVICE scalar_t bessel_j1_forward(scalar_t x) {
  ...
  SYCL_KERNEL_ASSERT(!"sycl::sqrt is not supported");
  /*return sign *
      (pp / pq *
...
static inline C10_HOST_DEVICE scalar_t bessel_y1_forward(scalar_t x) {
  ...
  SYCL_KERNEL_ASSERT(!"sycl::sin, cos, sqrt is not supported");
  /*return (pp / pq *
              std::sin(x - scalar_t(2.356194490192344928846982537459627163)) +
          scalar_t(5.0) / x * (qp / qq) *
              std::cos(x - scalar_t(2.356194490192344928846982537459627163))) *
